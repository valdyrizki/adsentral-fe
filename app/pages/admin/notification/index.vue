<template>
  <div class="space-y-6">
    ...
    <FormCreateNotificationModal v-model="showCreateModal" @submitted="refresh()" />

    <!-- Broadcast Modal -->
    <FormBroadcastNotificationModal v-model="showBroadcastModal" @submitted="refresh()" />
    ...
    <div class="flex items-center justify-between gap-2 flex-wrap">
      ...
      <div class="flex items-center gap-2">
        ...
        <UButton size="sm" color="warning" icon="i-heroicons-megaphone" @click="showBroadcastModal = true">
          Broadcast
        </UButton>
        <UButton size="sm" color="primary" icon="i-heroicons-plus" @click="showCreateModal = true">
          Buat Notifikasi
        </UButton>
...
definePageMeta({ layout: 'admin', label: 'Notifikasi' })

const toast = useToast()
const notificationStore = useNotificationStore()
const { fetchAdminNotifications, markAsRead, markAllAsRead } = useNotificationApi()
...
async function refresh() {
  ...
  error.value = null
  try {
    data.value = await fetchAdminNotifications(page.value, perPageValue.value)
  } catch (err: any) {
    error.value = err
  } finally {
    pending.value = false
