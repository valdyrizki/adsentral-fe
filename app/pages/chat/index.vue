            ...
            >
              ...
              <div v-else-if="!conversations?.content?.length" class="flex flex-col items-center justify-center py-16 px-4 text-center text-gray-400">
                ...
                <p class="text-xs mt-1 text-gray-400">
                  {{ conversationKeyword ? 'Coba kata kunci lain' : 'Percakapan dengan penjual akan muncul di sini' }}
                </p>
              </div>
                ...
                <div class="flex justify-between">
                  <div class="flex items-center gap-4">
                    <div class="flex-shrink-0">
                      <UAvatar
                        :src="getImageUrl(conversation?.merchant.logo_url)"
                        :chip="{
                          inset: true,
                          color: 'success'
                        }"
                    ...
                    <div class="min-w-0 flex-1">
                      <p class="truncate text-sm font-medium text-gray-900">{{ conversation?.merchant.name }}</p>
                      <p class="truncate text-sm text-gray-500">{{ conversation?.last_message }}</p>
                    </div>
                  </div>
                  <div class="flex flex-col items-end gap-2">
                    <UBadge v-if="conversation?.buyer_unread_count > 0" color="success" size="sm">{{conversation.buyer_unread_count}}</UBadge>
                ...
                <div>
                  ...
                  <div class="bg-gradient-to-r from-primary-500 to-primary-700 p-6 text-white border-b">
                    <div class="flex justify-between">
                      ...
                      <div class="flex-shrink-0">
                        <UAvatar
                          :src="getImageUrl(selectedConversation?.merchant.logo_url)"
                          :chip="{
                            inset: true,
                            color: 'success'
                          }"
                      ...
                      <div class="min-w-0 flex-1">
                        <p class="truncate text-lg font-medium text-white">{{ selectedConversation?.merchant.name }}</p>
                  ...
                  >
                    ...
                    <div v-for="chat in chatPages?.content" :key="chat.id">
                      <div
                        class=" flex"
                        :class="chat.sender_type === 'BUYER' ? 'justify-end' : 'justify-start'"
                      >
                        <div>
                          ...
                          <div v-if="chat.file" class="flex m-4"> <!-- File yang sudah diupload, klik untuk download -->
                            ...
                            <div v-else>
                              <UButton icon="mdi:download" color="primary" variant="soft" size="xs" @click="downloadFile(getImageUrl(chat.file?.url))">{{ chat.file?.ori_name }}</UButton>
                            </div>
                          </div>
                          <p
                            class="text-xs mt-1"
...
const { data: conversations, pending: pendingConversation, refresh: refreshConversations } = await useAsyncData<PageResponse<ConversationResponse>>(
  ...
  const loadChatByConversation = async (conversation: ConversationResponse) => {
    ...
    try {
      ...
      chatRequest.value.receiverId = conversation.merchant.id //Khusus menu buyer
...
const handleScrollConversation = async () => {
  ...
  if (container.scrollTop + container.clientHeight >= container.scrollHeight - 5) {
    ...
    const data = await fetchBuyerConversation(
      conversationPageValue.value,
      conversationPerPageValue.value,
      conversationKeyword.value
    )
